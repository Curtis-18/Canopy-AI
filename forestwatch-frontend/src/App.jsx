...
export default function App() {
  ...
  const SEARCH_FORESTS = useMemo(
    () => [
      ...
      { name: 'Kibale Forest', lat: 0.5, lng: 30.36 },
      { name: 'Bwindi Forest', lat: -1.03, lng: 29.68 },
      { name: 'Queen Elizabeth Nat. Park', lat: -0.2, lng: 29.9 },
    ],
    []
  )
  ...
  useEffect(() => {
    ...
    setSearchSuggestions(SEARCH_FORESTS.filter((f) => f.name.toLowerCase().includes(q)))
    ...
    const fetchData = async () => {
      ...
      const tileOpts =
        activeTab === 'analyze'
          ? {}
          : {
              date_start_a: ryA?.dateStart,
              date_end_a: ryA?.dateEnd,
              label_a: ryA?.label,
              date_start_b: ryB?.dateStart,
              date_end_b: ryB?.dateEnd,
              label_b: ryB?.label,
            }
      ...
      try {
        ...
        if (activeTab === 'analyze') {
          ...
          setCompareResult(analysisResult)
        }
        ...
        setError(
          `Could not reach backend. Check the Render URL and deployment status. ${err?.message || ''}`.trim()
        )
      ...
      <div style={{ display: 'flex', flex: 1, overflow: 'hidden', position: 'relative' }}>
              ...
              {activeTab === 'analyze' ? (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                  ...
                  >
                    ...
                    >
                      {YEAR_OPTIONS.map((o) => (
                        <option key={o.value} value={String(o.value)} style={{ color: '#000', background: '#fff' }}>
                          {o.label}
                        </option>
                      ))}
                    </select>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                  {[
                    ['BASELINE YEAR', yearA, setYearA],
                    ['COMPARE YEAR', yearB, setYearB],
                  ].map(([lbl, val, setter]) => (
                    <label
                      key={lbl}
                      style={{
        ...
        <div
          style={{
            ...
            flexDirection: 'column',
            transform:
              resultsOpen && (selectedPoint || loading || activeTab === 'hotzones')
                ? 'translateX(0)'
                : 'translateX(-100%)',
        ...
        >
          ...
          <div
            style={{
              padding: activeTab === 'hotzones' ? 0 : '20px',
              flex: 1,
              overflowY: 'auto',
              display: 'flex',
              flexDirection: 'column',
              gap: activeTab === 'hotzones' ? 0 : 16,
            }}
          >
            {error && (
              <div
